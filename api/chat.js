...
2. 대화 중 자연스럽게 다음 6가지 정보를 수집합니다:
   업장명, 업종(숙박/체험·레저/공예·예술/교육/식사·카페/기타), 담당자명, 연락처, 이메일, 제안내용
3. 6가지 정보가 모두 수집되면 아래 형식을 출력합니다:
[COMPLETE]
{"업장명":"...","업종":"...","담당자명":"...","연락처":"...","이메일":"...","제안내용":"...","대화요약":"..."}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const completeIdx = rawText.indexOf(COMPLETE_MARKER);
    const isComplete = completeIdx !== -1;

    let displayText = rawText;
    let data = null;

    if (isComplete) {
      displayText = rawText.slice(0, completeIdx).trim();
